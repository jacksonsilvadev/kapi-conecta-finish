...
const REDIS_KEY_DEFAULT = 'product:';
...
exports.getById = async (request, response) => {
  try {
    ...
    const productRedis = await redisClient.get(REDIS_KEY_DEFAULT + id);

    if (productRedis) {
      logger.info(`Returned cached value from product - ${id}`);
    ...
    await redisClient.set(REDIS_KEY_DEFAULT + id, product);
    return response.status(200).send(product);
  } catch (e) {
    logger.error(e);
    return response
