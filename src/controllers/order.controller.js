...
exports.getById = async (request, response) => {
  try {
    ...
    const orderRedis = await redisClient.get(REDIS_KEY_DEFAULT + id);

    if (orderRedis) {
      logger.info(`Returned cached value from order - ${id}`);
      return response.status(200).send(orderRedis);
    }
    ...
    await redisClient.set(REDIS_KEY_DEFAULT + id, order);
...
exports.create = async (request, response) => {
  try {
    ...
    sendToQueue('report', order);
    sendToQueue('email', order);
...
exports.delete = async (request, response) => {
  try {
    ...
    await redisClient.del(REDIS_KEY_DEFAULT + id);
